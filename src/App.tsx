...
export const App: React.FC = () => {
  ...
  const handleSaveIngredient = async (ingredientData: Partial<Ingredient>) => {
    ...
    const isEditing = !!ingredientData.id;
    const idToSave = ingredientData.id || generateId();
    
    if (!ingredientData.image_url) {
        const nameForPlaceholder = getIngredientName(ingredientData as Ingredient, currentUser.preferred_language);
        ingredientData.image_url = placeholderImage(DEFAULT_IMAGE_SIZE, DEFAULT_IMAGE_SIZE, nameForPlaceholder);
    }

    const apiIngredientData: Ingredient = {
        id: idToSave, 
        name_localized: ingredientData.name_localized!,
    ...
    try {
        if (isEditing) {
            await updateIngredientAPI(apiIngredientData.id, apiIngredientData);
        } else {
  ...
  const handleSaveDish = async (dishData: Omit<Dish, 'id'> & {id?: string}) => {
    ...
    const newNameLocalized: LocalizedText = dishData.name_localized;
    const newPrepStepsLocalized: LocalizedText = dishData.preparation_steps_localized;
  ...
  const handleSaveCookingItem = async (itemData: Omit<CookingItem, 'id'> & {id?: string}) => {
    ...
    const newNameLocalized: LocalizedText = itemData.name_localized;
    const newSummaryLocalized: LocalizedText = itemData.summary_localized;
  ...
  const handleSaveCustomer = async (submittedCustomerData: Customer) => {
    ...
    try {
        ...
        if (editingItem && editingItem.id) {
            savedCustomerRecord = await updateCustomerAPI(editingItem.id, customerUpdatePayload);
        } else {
  ...
  const handleUpdateProfile = async (profileData: Customer) => {
    ...
    try {
        ...
        setIsSettingsMenuOpen(false); // Close menu on success
    } catch (error) {
  ...
  const calculateTotalIngredientsForCustomer = (customer: Customer): CumulativeIngredient[] => {
    const aggregatedIngredients: { [ingredientId: string]: { totalQuantity: number; unit: string; masterIngredientId: string; name: string; pricePerBaseUnit: number; } } = {};
            ...
            dish.ingredients.forEach(dishIngredient => {
                ...
                if (masterIngredient) {
                    ...
                    if (!aggregatedIngredients[masterIngredient.id]) {
                        aggregatedIngredients[masterIngredient.id] = { totalQuantity: 0, unit: masterBaseUnit, masterIngredientId: masterIngredient.id, name: getIngredientName(masterIngredient, currentUser?.preferred_language || Language.EN), pricePerBaseUnit: pricePerMasterBaseUnit };
                    }
  ...
  const renderPage = () => {
    switch (currentPage) {
        ...
        case Page.Home: return currentUser?.role === UserRole.SUPREM ? <SupremDashboard 
                users={users} 
                ingredients={ingredients.map(i => ({...i, name: getIngredientName(i, currentLanguage)}))}
                dishes={dishes} 
    ...
    <div className="min-h-screen bg-slate-100 text-slate-800">
              ...
              <div ref={settingsMenuRef} className="relative">
                ...
                {isSettingsMenuOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-xl overflow-hidden z-20 ring-1 ring-slate-200">
                        <button
                            onClick={() => { openModal('profile'); setIsSettingsMenuOpen(false); }}
      ...
      {modalOpen && modalType && (
        <Modal onClose={closeModal}>
          ...
          {modalType === 'profile' && currentUser && <CustomerForm onSave={handleUpdateProfile} onCancel={closeModal} existingCustomer={{...currentUser, name: currentUser.username, image_url: currentUser.image_url, number_of_persons:0, selectedDishes: [], selectedCookingItems: [], generated_order_details: null}} isProfileForm={true} dishes={[]} cookingItems={[]} ingredients={ingredients} generateId={generateId} currentUserPreferredLanguage={currentLanguage} isSubmitting={isModalSubmitting}/>}
