...
import { CumulativeIngredient, Ingredient, IngredientUnits, Language, LanguageLabels, UITranslationKeys } from '../types';
...
interface OrderIngredientFormProps {
    onSave: (data: {
        customerId: string;
        orderLineItemId?: string; // ID of the CumulativeIngredient to update (if editing)
        masterIngredientId?: string; // ID of the master ingredient (if adding new)
        quantity: number;
        unit: string;
        isAdding: boolean;
    }) => void;
    ...
    context: {
        customerId: string;
        orderLineItemId?: string; // ID of the CumulativeIngredient being edited
        isAdding: boolean;
        // For editing, pass data of the item being edited to pre-fill the form
        existingOrderItemData?: { name: string; quantity: number; unit: string; masterIngredientId: string };
    };
    masterIngredients: Ingredient[]; // Full list of master ingredients to select from when adding
...
const OrderIngredientForm: React.FC<OrderIngredientFormProps> = ({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isAdding && !selectedMasterIngredientId) {
        ...
        onSave({
            ...
            masterIngredientId: isAdding ? selectedMasterIngredientId : undefined,
            quantity,
            unit,
            isAdding,
        });
    ...
    const formTitle = isAdding
        ? getUIText(UITranslationKeys.ADD_ORDER_INGREDIENT_TITLE, currentUserPreferredLanguage)
        : getUIText(UITranslationKeys.EDIT_ORDER_INGREDIENT_TITLE, currentUserPreferredLanguage);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4" aria-labelledby="order-ingredient-form-title">
            ...
            {isAdding ? (
                <div>
                    <label htmlFor="master-ingredient-select" className="block text-sm font-medium text-slate-700">
                        {getUIText(UITranslationKeys.MASTER_INGREDIENT_LABEL, currentUserPreferredLanguage)}
                    </label>
                    <select
                        id="master-ingredient-select"
                        value={selectedMasterIngredientId}
                        onChange={(e) => setSelectedMasterIngredientId(e.target.value)}
                    ...
                    >
                        ...
                        {masterIngredients.map(ing => (
                            <option key={ing.id} value={ing.id}>
                                {getTranslatedText(ing.name, currentUserPreferredLanguage)}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div>
                    <p className="block text-sm font-medium text-slate-700">
                      {getUIText(UITranslationKeys.ORDER_INGREDIENT_NAME_LABEL, currentUserPreferredLanguage)}
                    </p>
            ...
            <div>
                <label htmlFor="order-ingredient-quantity" className="block text-sm font-medium text-slate-700">
                    {getUIText(UITranslationKeys.QUANTITY_LABEL, currentUserPreferredLanguage)}
                </label>
                <input
                    id="order-ingredient-quantity"
                    type="number"
                    value={quantity}
            ...
            <div>
                <label htmlFor="order-ingredient-unit" className="block text-sm font-medium text-slate-700">
                    {getUIText(UITranslationKeys.UNIT_LABEL, currentUserPreferredLanguage)}
                </label>
                <select
                    id="order-ingredient-unit"
                    value={unit}
